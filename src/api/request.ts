...
export function buildQueryStringParts(
	key: Array<string>,
	value: IRequestQuery,
	level: number = 0
) {
	if (level > 16)
		throw new Error("Query string nesting level of 16 exceeded.");
...
export function buildQueryString(
	...
	if (parts.length > 0) return prefix + parts.join(delimiter);
	else return "";
}
...
export function mockRequest<T = null>(
	...
	return new Promise((resolve, reject) => {
		...
		setTimeout(() => {
			...
			if (responseStatus === HTTP_STATUS.OK) resolve(res);
			else reject(new ErrorResponse(res));
		}, delay);
	});
}
...
export function apiRequest<T = null>(
	...
	path: string,
	query: IRequestQuery = null,
	body: IRequestBody | FormData = null
): Promise<IResponse<T>> {
	return new Promise((resolve, reject) => {
		const url =
			CONFIG.api_url +
			path +
			(query !== null ? buildQueryString(query) : "");
		const req = new HttpRequest(method, url, query, body);
